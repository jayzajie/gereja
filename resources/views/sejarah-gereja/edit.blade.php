...
@section('title', 'Edit Sejarah Gereja Toraja')
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0 text-gray-800">✏️ Edit Sejarah Gereja Toraja</h1>
            <p class="mb-0 text-muted">Perbarui informasi sejarah gereja</p>
        </div>
    ...
    <div class="card shadow">
        ...
        <div class="card-body">
            <form action="{{ route('sejarah-gereja.update', $sejarahGereja) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" class="form-control @error('title') is-invalid @enderror" 
                           value="{{ old('title', $sejarahGereja->title) }}" placeholder="Contoh: Sejarah Gereja Toraja">
                    @error('title')
                ...
                <div class="form-group">
                    ...
                    <textarea id="content" name="content" class="form-control large-textarea @error('content') is-invalid @enderror"
                              rows="10" placeholder="Tulis sejarah gereja di sini...">{{ old('content', $sejarahGereja->content) }}</textarea>
                    @error('content')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-row">
                    ...
                    <!--
                    <div class="form-group">
                        <label for="banner_image">Banner Gereja</label>
                        <div class="file-upload-container">
                            <input type="file" id="banner_image" name="banner_image" class="file-input @error('banner_image') is-invalid @enderror"
                                   accept="image/*">
                            <div class="file-upload-text">
                                <i class="fas fa-cloud-upload-alt"></i>
                                <span>Pilih banner gereja (JPG, PNG, GIF - Max: 5MB)</span>
                            </div>
                        </div>
                        @if($sejarahGereja->banner_image)
                            <div class="current-file mt-2">
                                <small class="text-muted">File saat ini:</small>
                                <img src="{{ $sejarahGereja->banner_image_url }}" alt="Current Banner" class="img-thumbnail" style="max-height: 100px;">
                            </div>
                        @endif
                        @error('banner_image')
                            <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="form-group">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" 
                               {{ old('is_active', $sejarahGereja->is_active) ? 'checked' : '' }}>
